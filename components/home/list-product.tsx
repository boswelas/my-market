...
export default function ListProduct({ title, price, created_at, photo, id }: ListProductProps) {
    return (
        <Link href={`/products/${id}`} className="">
            <div className="flex md:flex-col bg-neutral-800 rounded-lg w-80 sm:w-96 md:w-[17em] md:h-96 shadow-xl hover:shadow-2xl">
                ...
                <div className="flex flex-col ml-5 md:ml-0  md:items-center md:text-center justify-center text-white p-2 w-full">
                    <span className="text-lg font-normal w-[9em] sm:w-[10.5em] overflow-hidden text-ellipsis whitespace-nowrap">
                        {title}
                    </span>
